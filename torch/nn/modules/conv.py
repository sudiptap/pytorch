...
class Conv2d(_ConvNd):
    ...
    In the simplest case, the output value of the layer with input size
    :math:`(N, C_{\text{in}}, H, W)` and output :math:`(N, C_{\text{out}}, H_{\text{out}}, W_{\text{out}})` can be
    precisely described as:
...
class Conv3d(_ConvNd):
    ...
    In the simplest case, the output value of the layer with input size
    :math:`(N, C_{\text{in}}, D, H, W)` and output :math:`(N, C_{\text{out}}, D_{\text{out}}, H_{\text{out}}, W_{\text{out}})` can be
    precisely described as:
    ...
    :math:`N` is a batch size, :math:`C` denotes a number of channels,
    :math:`D` is a depth of input planes, :math:`H` is a height of input planes in pixels,
    and :math:`W` is width in pixels.
    ...
    * :attr:`dilation` controls the spacing between the kernel points; also
    ...
    Args:
        ...
        kernel_size (int or tuple): Size of the convolving kernel
        stride (int or tuple, optional): Stride of the convolution. Default: 1
        padding (int, tuple or str, optional): Padding added to all three sides of
            the input. Default: 0
        padding_mode (str, optional): ``'zeros'``, ``'reflect'``,
            ``'replicate'`` or ``'circular'``. Default: ``'zeros'``
        dilation (int or tuple, optional): Spacing between kernel elements. Default: 1
...
class ConvTranspose1d(_ConvTransposeNd):
    ...
    * :attr:`dilation` controls the spacing between the kernel points; also known as the à trous algorithm.
    ...
    Examples::
        ...
        >>> # non-square kernels and unequal stride and with padding
        >>> m = nn.ConvTranspose1d(16, 33, (3, 5), stride=(2, 1), padding=(4, 2))
        >>> input = torch.randn(20, 16, 50)
        >>> output = m(input)
